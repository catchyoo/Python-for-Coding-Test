...
25) 문자열과 관련된 꿀팁 함수!!!!!!!!!!!!!!!! **1. count 함수** ex) s = s.count('0') 라고 쓰면, s 안의 0의 개수 세줌 / **2. replace 함수** ex) s = s.replace('0','')라고 쓰면, s안의 0를 없애      줌. / **3. bin 함수** ex) s = bin(len(s))[2:] 라고 쓰면, s의 길이를 다시 이진수로 바꾸어줌. 이때 [2:] 꼭 사용하기! 앞에 0b가 붙기 때문.
